  ...
  if (active && payload && payload.length) {
    return (
      <Box sx={{
        ...
        <Typography variant="caption" sx={{ color: '#004D40', fontWeight: 'bold', display: 'block', mb: 0.5 }}>
          Tahun {label}
        </Typography>
        <Typography variant="body2" sx={{ color: '#004D40' }}>
          <strong>{payload[0].value.toLocaleString('id-ID', { maximumFractionDigits: 2 })} ha</strong>
        </Typography>
      </Box>
    );
  }
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <Box sx={{
        ...
        <Typography variant="caption" sx={{ color: '#455a64', fontWeight: 'bold', display: 'block', mb: 0.5 }}>
          Tahun {label}
        </Typography>
        <Typography variant="body2" sx={{ color: val >= 0 ? '#00695C' : '#c62828', fontWeight: 'bold' }}>
          {val > 0 ? '+' : ''}{val.toLocaleString('id-ID', { maximumFractionDigits: 2 })} ha
        </Typography>
      </Box>
    );
  }
...
const SummaryCard = ({ label, value, unit, year, icon: Icon, color, bgColor, borderColor }) => (
  <Paper elevation={0} sx={{
    ...
    <Box>
      ...
      <Typography variant="h5" sx={{ color, fontWeight: 900, mt: 0.5, lineHeight: 1 }}>
        {value}
      </Typography>
      <Typography variant="caption" sx={{ color: '#90a4ae' }}>{unit}</Typography>
    </Box>
  </Paper>
);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <SummaryCard
            ...
            value={summary.max_area?.area_ha.toLocaleString('id-ID', { maximumFractionDigits: 2 })}
            unit="hektar"
            icon={TrendingUpIcon}
            color="#00695C"
            bgColor="#E0F2F1"
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <SummaryCard
            ...
            value={summary.min_area?.area_ha.toLocaleString('id-ID', { maximumFractionDigits: 2 })}
            unit="hektar"
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <SummaryCard
            ...
            value={summary.biggest_loss_epoch?.delta_ha?.toLocaleString('id-ID', { maximumFractionDigits: 2 })}
            unit="hektar"
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4 }}>
        ...
        <Typography variant="caption" sx={{ color: '#78909c', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, whiteSpace: 'nowrap' }}>
          Visualisasi Data
        </Typography>
        <Divider sx={{ flex: 1, borderColor: '#b2dfdb' }} />
      </Box>
      ...
      <Grid container spacing={4}>
        ...
        <Grid size={{ xs: 12 }}>
          <Paper elevation={0} sx={{
            ...
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" sx={{ color: '#004D40', fontWeight: 800 }}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 10 }}>
                  ...
                  <Line
                    ...
                    stroke="#00BFA5"
                    strokeWidth={3.5}
                    dot={{ r: 5, fill: '#004D40', stroke: '#00BFA5', strokeWidth: 2 }}
                    activeDot={{ r: 8, fill: '#004D40', stroke: '#fff', strokeWidth: 2 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            </Box>
            ...
            <Box sx={{ mb: 3, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
                  <Box sx={{ width: 12, height: 12, borderRadius: 1, bgcolor: '#00BFA5' }} />
                  <Typography variant="caption" sx={{ color: '#455a64', fontWeight: 600 }}>Gain</Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
                  <Box sx={{ width: 12, height: 12, borderRadius: 1, bgcolor: '#EF5350' }} />
                  <Typography variant="caption" sx={{ color: '#455a64', fontWeight: 600 }}>Loss</Typography>
                </Box>
              </Box>
            </Box>
            <Box sx={{ width: '100%', height: 360 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData.slice(1)} margin={{ top: 10, right: 30, left: 10, bottom: 10 }} barSize={36}>
                  ...
                  <YAxis
                    ...
                    width={80}
                  />
                  <RechartsTooltip content={<BarTooltip />} cursor={{ fill: 'rgba(0,0,0,0.04)' }} />
