...
const Sidebar = ({
  ...
  const exportCSV = () => {
    ...
    const rows = stats.epochs
      .map(
        (e) => `${e.year},${e.area_ha},${e.delta_ha || 0},${e.delta_pct || 0}`,
      )
      .join("\n");
    ...
    >
      ...
      <FormControl fullWidth sx={{ mb: compareMode ? 2 : 4 }}>
        <InputLabel id="year-select-label">
          {compareMode ? "Tahun Dasar" : "Tahun Epoch"}
        </InputLabel>
        <Select
      ...
      {compareMode && (
        <FormControl fullWidth sx={{ mb: 4 }}>
          <InputLabel id="compare-year-select-label">
            Tahun Pembanding
          </InputLabel>
          <Select
      ...
      <FormControlLabel
        ...
        label={
          <Typography variant="body2">
            Mode Perbandingan (Change Detection)
          </Typography>
        }
        sx={{ mb: 1 }}
      />
      ...
      <Box
        sx={{ p: 2, bgcolor: "rgba(0, 191, 165, 0.1)", borderRadius: 2, mb: 2 }}
      >
        ...
        <Typography variant="h4" sx={{ color: "#004D40", fontWeight: "bold" }}>
          {currentEpochStat
            ? currentEpochStat.area_ha.toLocaleString("id-ID", {
      ...
      {!compareMode &&
        currentEpochStat &&
        currentEpochStat.delta_ha != null &&
        currentEpochStat.delta_ha !== 0 && (
          ...
          >
            ...
            <Typography
              variant="h5"
              sx={{
                color: currentEpochStat.delta_ha > 0 ? "#2E7D32" : "#C62828",
                fontWeight: "bold",
              }}
            >
              {currentEpochStat.delta_ha > 0 ? "+" : ""}
              {currentEpochStat.delta_ha.toLocaleString("id-ID", {
                maximumFractionDigits: 2,
              })}{" "}
              ha
              {currentEpochStat.delta_pct != null && (
                <>
                  {" "}
                  ({currentEpochStat.delta_pct > 0 ? "+" : ""}
                  {currentEpochStat.delta_pct.toFixed(2)}%)
                </>
              )}
            </Typography>
          </Box>
      ...
      <Typography
        variant="subtitle1"
        sx={{ fontWeight: "bold", mb: 2, mt: "auto" }}
      >
        Tren Perubahan Luas
      </Typography>
