...
import { Box, Typography, Container, CircularProgress, Alert, Chip } from '@mui/material';
...
const Chart = () => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 64px)' }}>
      ...
      {/* Hero Header Banner */}
      <Box sx={{
        background: 'linear-gradient(135deg, #004D40 0%, #00695C 50%, #0277BD 100%)',
        py: { xs: 5, md: 7 },
        px: 2,
        position: 'relative',
        overflow: 'hidden',
        ...
        <Container maxWidth="lg">
          ...
          >
            <Chip
              icon={<BarChartIcon sx={{ color: '#A5F3FC !important', fontSize: '0.85rem' }} />}
      ...
      <Box sx={{ flex: 1, bgcolor: '#f0f4f3', py: 5 }}>
        <Container maxWidth="lg">
          {loading && (
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, p: 10 }}>
              <CircularProgress sx={{ color: '#00BFA5' }} size={48} />
              <Typography variant="body2" color="text.secondary">Memuat data statistik...</Typography>
            </Box>
          )}
