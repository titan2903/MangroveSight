...
def get_mangrove_geojson(
    ...
    simplify: bool = Query(
        False, description="Apakah geometri disederhanakan untuk zoom rendah?"
    ),
...
@router.get("/heatmap")
def get_mangrove_heatmap(
    year: int = Query(..., description="Tahun epoch"), db: Session = Depends(get_db)
):
    """
